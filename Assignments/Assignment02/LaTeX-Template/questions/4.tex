...
\begin{table}[H]
    ...
    \begin{tabular}{rl}
    \hline
    \textbf{مسئولیت} & \textbf{\lr{Use Case}} \\
    \hline
...
\begin{itemize}
  \item \textbf{\lr{Controllers}}: \lr{HTTP request} را دریافت، اعتبارسنجی، و به \lr{DTO} تبدیل می‌کنند
  \item \textbf{\lr{Presenters}}: خروجی \lr{use case} را به فرمت \lr{JSON} مناسب برای کلاینت تبدیل می‌کنند
  \item \textbf{\lr{Repository Implementations}}: پیاده‌سازی واقعی \lr{interface} های \lr{Domain} با استفاده از \lr{ORM}
  \item \textbf{\lr{External Gateways}}: پیاده‌سازی \lr{Port Interface} ها 
  \item \textbf{\lr{Middleware}}: \lr{Auth Guard}، \lr{Rate Limiter}، \lr{Logger}، \lr{Error Handler}
\end{itemize}
...
\begin{tabular}{rrl}
\hline
\textbf{بخش} & \textbf{توضیح} & \textbf{فناوری} \\
\hline
