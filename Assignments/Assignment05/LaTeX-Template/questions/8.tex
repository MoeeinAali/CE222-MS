 کل پیکربندی به‌جای کلیک در رابط گرافیکی، از طریق \lr{REST API} نکسوس و در قالب اسکریپت انجام شده تا تکرارپذیر و قابل بازبینی باشد.
...
\subsubsection*{دریافت وابستگی‌ها از مخزن ساخته‌شده:}

پروژه‌ی سؤال ۴ برای این کار استفاده شد؛ چون همان‌جا سازوکار \lr{MIRROR\_URL} پیاده شده بود، کافی بود نشانی نکسوس به‌عنوان \lr{index} داده شود و مرحله‌ی \lr{builder} همان‌جا هدف قرار بگیرد:
...
docker build \
  ...
  --target builder -t q4-app:from-nexus ../Q4-dockerize
\end{lstlisting}

نکته: چون مخزن نکسوس روی \lr{HTTP} ساده است (نه \lr{HTTPS})، بدون \lr{MIRROR\_TRUSTED\_HOST} خود \lr{pip} با پیغام «\lr{not a trusted or secure host}» نصب را رد می‌کند؛ به همین دلیل این آرگومان در Dockerfile سؤال ۴ به \lr{pip install --trusted-host} ترجمه می‌شود.

\subsubsection*{ پوش و پول ایمیج}
...
\subsubsection*{ نقش‌های نکسوس در یک پایپلاین \lr{CI/CD}}
...
\begin{enumerate}
	...
	\item {کنترل دسترسی}: نقش و کاربر جداگانه برای \lr{CI} (فقط خواندن از \lr{proxy}ها و نوشتن در \lr{hosted})، به‌همراه لاگ اینکه چه کسی چه چیزی را چه زمانی منتشر کرده است.
...
\subsubsection*{کاربرد مخزن نوع \lr{Group}}
