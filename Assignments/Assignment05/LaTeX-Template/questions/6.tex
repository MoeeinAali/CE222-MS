کد این بخش در \lr{solutions/Q6-compose/} قرار دارد.

\subsectionaddtolist{الف) معماری}
...
\subsectionaddtolist{ب) رعایت محدودیت‌های صورت سؤال}
...
\hline
\lr{nginx} بین جاوا و سرویس کمکی & جداسازی شبکه‌ها (بند الف) \\
\hline
...
\subsectionaddtolist{ج) \lr{docker-compose.yaml}}
...
\subsectionaddtolist{د) \lr{nginx.conf}}
...
سرویس کمکی هم با یک \lr{Dockerfile} کوچک روی \lr{python:3.12-alpine} با کاربر بدون امتیاز بسته‌بندی شده است؛ \lr{EXPOSE 9090} در آن فقط مستندسازی است و هیچ پورتی روی میزبان باز نمی‌کند.

\subsectionaddtolist{ه) اجرا و دو خط لاگ}
...
برای اینکه نشان دهیم \lr{nginx} واقعا در مسیر است (و نه صرفا کنار آن)، از یک کانتینر روی \lr{edge-net} — همان شبکه‌ای که برنامه‌ی جاوا در آن است — یک‌بار مستقیم به \lr{helper} و یک‌بار از طریق \lr{nginx} درخواست زده شد:
...
نکات کاربردی:
\begin{itemize}
	\item ترتیب اولویت از کم به زیاد: مقدار پیش‌فرض در \lr{Dockerfile} (\lr{ENV}) $\rightarrow$ \lr{env\_file} $\rightarrow$ کلید \lr{environment} $\rightarrow$ متغیر پاس‌داده‌شده در خط فرمان.
	\item چند \lr{env\_file} مجاز است و در تعارض، فایل بعدی برنده است؛ همین اجازه می‌دهد پیکربندی را موضوعی تفکیک کنیم (اینجا: تنظیمات برنامه و تنظیمات ردیس).
...
\subsectionaddtolist{ز) انواع \lr{Network} در داکر}
...
{درایور} & {توضیح} \\
\hline
\lr{bridge} & پیش‌فرض؛ شبکه‌ی خصوصی روی یک میزبان. شبکه‌ی \lr{bridge} کاربرساخته \lr{DNS} داخلی \\
