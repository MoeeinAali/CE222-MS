کد این بخش در \lr{solutions/Q5-react-optimize/} قرار دارد. پروژه یک برنامه‌ی \lr{React 18} با \lr{TypeScript} و \lr{Vite} است (داشبورد وضعیت سرویس‌ها) و طبق صورت سؤال {هیچ خطی از کد برنامه تغییر نکرده}؛ تنها فایل‌های مرتبط با داکر اضافه یا اصلاح شده‌اند.
...
\subsectionaddtolist{الف) مشکلات \lr{Dockerfile} اولیه}
...
\subsectionaddtolist{ب) نسخه‌ی بهینه‌شده}
...
\subsectionaddtolist{ج) مقایسه‌ی حجم و سرعت}
...
\subsectionaddtolist{د) توضیح تغییراتی که باعث بهینه‌سازی شدند}
...
\begin{enumerate}
	...
	\item {ترتیب لایه‌ها}: \lr{package.json} و \lr{package-lock.json} جدا و {قبل} از سورس کپی می‌شوند؛ تا وقتی \lr{lockfile} عوض نشود، لایه‌ی نصب از کش می‌آید. این تنها دلیل کاهش ۱۲٫۷ ثانیه به ۱٫۹ ثانیه است.
...
\subsectionaddtolist{ه) \lr{docker-compose.yml} و \lr{nginx.conf}}
...
services:
  web:
    ...
    healthcheck:
      ...
      timeout: 3s
      retries: 5
    read_only: true            # nginx only needs writable temp dirs
    tmpfs:
      - /var/cache/nginx
      - /var/run
    security_opt:
...
server {
    ...
    # the entry document must never be cached, otherwise clients keep
    # pointing at deleted asset hashes after a deploy
    location = /index.html { add_header Cache-Control "no-cache"; }

    # single page app: unknown paths are handled by the client router
