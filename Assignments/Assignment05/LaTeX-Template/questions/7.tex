کد این بخش در \lr{solutions/Q7-pipeline/} قرار دارد.

\subsectionaddtolist{الف) پروژه‌ی پایه}

یک سرویس \lr{HTTP} به زبان \lr{Go 1.23} با دو نقطه‌ی انتهایی \lr{GET /health} و \lr{POST /v1/analyze} (آمار واژگان یک متن) نوشته شد؛ ۱۵ تست واحد با پوشش ۷۰٫۵ درصدی کل و ۱۰۰ درصدی بسته‌ی منطق دامنه. تنها وابستگی بیرونی \lr{github.com/google/uuid} است تا مرحله‌ی \lr{install} کار واقعی داشته باشد. سرویس \lr{graceful shutdown} دارد و شماره‌ی نسخه‌اش در زمان بیلد با \lr{-ldflags} روی باینری حک می‌شود؛ همین باعث می‌شود در مرحله‌ی \lr{deploy} بتوان تأیید کرد که دقیقا همان \lr{artifact} این پایپلاین اجرا شده است.

\subsectionaddtolist{ب) مدل اجرای رانر}
...
in_docker() {                     # one `script:` block of a GitLab job
    local image="$1"; shift
    docker run --rm \
        -v "$WORKSPACE:/src" \
...
\subsectionaddtolist{د) امکاناتی که از \lr{GitLab CI} تقلید شده‌اند}
...
\subsectionaddtolist{ه) یک نکته‌ی مهم در \lr{bash}}

در نسخه‌ی اول، جاب \lr{build} با وجود شکست \lr{docker build} سبز گزارش می‌شد. علت این است که \lr{set -e} در بدنه‌ی تابعی که به‌عنوان {شرط \lr{if}} اجرا می‌شود موقتا غیرفعال می‌گردد؛ بنابراین دستور شکست‌خورده‌ی وسط تابع، اجرا را متوقف نمی‌کرد و مقدار بازگشتی تابع، وضعیت {آخرین} دستور بود. راه‌حل، افزودن \lr{|| return 1} صریح به انتهای هر گام است. این دقیقا همان دسته خطاهایی است که یک پایپلاین را سبز دروغین می‌کند و به همین دلیل، خود سناریوهای شکست هم باید آزموده شوند.

\subsectionaddtolist{و) نتیجه‌ی اجرا — حالت موفق}
...
\subsectionaddtolist{ز) نتیجه‌ی اجرا — حالت شکست هر مرحله}
