...
\begin{enumerate}
    \item {مرحله برنامه‌ریزی و طراحی:} ابتدا نیازهای پروژه تعریف شد، شامل پشتیبانی از تنظیمات \lr{YAML} ، انواع مسیرها (ریدایرکت، سرو فایل، پروکسی معکوس، سرو دایرکتوری) و استفاده از الگوی استراتژی برای هندلرها. معماری لایه‌ای انتخاب شد تا نگرانی‌ها جدا شوند.
    
    \item {پیاده‌سازی بارگذاری تنظیمات:} فایل \texttt{config.go} نوشته شد برای خواندن فایل \lr{YAML} و تبدیل به ساختار داده‌ای. این شامل تعریف \lr{struct} های \lr{Config} و \lr{Route} بود.
    
    \item {پیاده‌سازی سرور و مدیریت اتصالات:} در \texttt{server.go}، \lr{listener TCP} روی پورت مشخص ایجاد شد و برای هر اتصال ورودی یک goroutine اختصاص یافت تا concurrency پشتیبانی شود.
    
    \item {پیاده‌سازی روتینگ و دیسپچ:} در \texttt{router.go}، پارسر خط درخواست اضافه شد، مسیرها با استفاده از پیشوند تطبیق داده شدند و با switch بر اساس نوع مسیر، دیسپچ به هندلر انجام شد.
    
    \item {پیاده‌سازی هندلرها:} چهار هندلر مستقل نوشته شد: \lr{Redirect}، StaticFile، ServeDir، \lr{ReverseProxy}. هر کدام با استراتژی ساده پیاده‌سازی شدند.
    
    \item {افزودن ویژگی‌های کمکی:} لاگینگ به فایل و کنسول اضافه شد، و تست دستی با ابزارهایی مانند curl انجام شد.
