...
کل سامانه یک \lr{Bounded Context} است و الگوهای تاکتیکی زیر در آن پیاده شده‌اند: \lr{aggregate} غنی \lr{Link} با فیلدهای خصوصی و \lr{invariant} «لینک غیرفعال را نمی‌توان دوباره غیرفعال کرد»؛ \lr{Value Object}های خوداعتبارسنج (\lr{URL}، \lr{ShortCode}، \lr{LinkID} و \lr{LinkStatus})؛ رویدادهای دامنه (\lr{LinkCreated}، \lr{LinkDisabled} و \lr{LinkClicked})؛ و الگوی \lr{Ports \& Adapters} که در آن دامنه هیچ وابستگی‌ای به زیرساخت ندارد (\lr{Persistence Ignorance}).
